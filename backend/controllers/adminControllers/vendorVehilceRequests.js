...
export const fetchVendorVehilceRequests = async (req,res,next) => {
    ...
    if(!vendorRequests){
    next(errorHandler(500,"something went wrong while fetching vendor requests"))
    }
    if(vendorRequests){
  ...
  export const approveVendorVehicleRequest = async (req,res,next) => {
    try{
     ...
     const approvedVendor  = await Vehicle.findByIdAndUpdate(_id, { isAdminApproved: true }, {
      new: true, 
    })
  ...
  export const rejectVendorVehicleRequest = async (req, res, next)=> {
    try{
        ...
        const {_id} = req.body
        const regectedVendor = await Vehicle.findByIdAndUpdate(_id, { isRejected: true }, {
          new: true, 
        })
